...
<?php
    ...
    if(isset($_POST['qualification_submit_btn'])){
        ...
        if(empty($qualification || $discipline || $schcategory || $institution || $graduationyear || $grade || $gradedescription )){
            ...
            */
               $query = "INSERT INTO qualifications(job_id, highestqualification, discipline, schcategory, institution, graduationyear, grade, gradedescription, sess_check) VALUES('{$job_id}','{$qualification}','{$discipline}','{$schcategory}','{$institution}','{$graduationyear}','{$grade}','{$gradedescription}','{$check}')";
