    ...
    if(isset($_POST['career_submit_btn'])){
        ...
        if(empty($lastempname || $lastempaddress || $lastjobposition || $lastjobrolecategory || $lastempyear || $currentempstatus)){
            ...
            $job_id = $acronym . $increase_num;
               ...
               $query = "INSERT INTO career_workexperience(job_id, lastempname, lastempaddress, lastjobposition, jobrolecategory, lastempyear, currentempstatus, sess_check) VALUES('{$job_id}','{$lastempname}','{$lastempaddress}','{$lastjobposition}','{$lastjobrolecategory}','{$lastempyear}','{$currentempstatus}','{$check}')";
