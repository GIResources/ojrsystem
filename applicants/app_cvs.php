...
<?php
    ...
    if(isset($_POST['cvs_submit_btn'])){
         ...
         // Testing and formating Applicant's Curriculum Vitae and Resume for Insertion the Database and Upload to different certificate folders ////
        
                    if (!isset($_FILES['appCvs']['tmp_name'])) {
                    echo "";
                    }else{
                    $file=$_FILES['appCvs']['tmp_name'];
                    $image= addslashes(file_get_contents($_FILES['appCvs']['tmp_name']));
                    $image_name= addslashes($_FILES['appCvs']['name']);
                    if ($image==FALSE) {
                            ...
                            move_uploaded_file($_FILES["appCvs"]["tmp_name"],"app_cvs/" . $_FILES["appCvs"]["name"]);

                            $appCvs="app_cvs/" . $_FILES["appCvs"]["name"];
                        
        // Testing and formating Applicant's Curriculum Vitae and Resume for Insertion the Database and Upload to different certificate folders ////
                        
               $query = "INSERT INTO cvs(job_id, appCvs, sess_check) VALUES('{$job_id}','{$appCvs}','{$check}')";
                        
                    $run_query = mysqli_query($connection, $query);

                        if($run_query == true){
                            echo "Congratulation! Your CV Upload and Submission was Successful!";
                            }else{
                                echo "Ooops! Sorry, Your CV Upload and Submission Attempt Failed. Try again!";
                            }
            header("Location: feedback5.php");
                        }
    ...
    <div class="container-fluid bg-success">
        ...
        <div class="col-xs-6 col-sm-6 col-md=6">
                <div class="panel panel-danger">
                    <div class="panel-heading">
                        <h4><p align="center"><strong class="text-info">CURRICULUM VITAE SECTION</strong></p></h4>
                    </div>
        ...
        <form id="form" method="POST" action="" enctype="multipart/form-data" style="padding-left: 20px; padding-right:20px; color: blue; width:70%; ">
                    <p><b>Upload Your Curriculum Vitae or Resume</b></p>
                    <input type="file" class="form-control" name="appCvs" required /><br />
                                
                    <p align="center"><input type="submit" name="cvs_submit_btn" value="NEXT" class="btn btn-info" /></p>
