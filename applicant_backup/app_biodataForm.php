...
<?php
    ...
    if(isset($_POST['biodata_submit_btn'])){
        ...
        if(empty($firstname || $lastname || $raddress || $phone || $email || $gender || $d_o_b || $maritalstatus || $state || $localgov || $nationality || $hobbies )){
           ...
           $query = "INSERT INTO app_biodataform(job_id,firstname,lastname,raddress,phone,email,gender,dateofbirth,maritalstatus,state,localgov,nationality,hobbies)
           VALUES('{$job_id}','{$firstname}','{$lastname}','{$raddress}','{$phone}','{$email}','{$gender}','{$d_o_b}','{$maritalstatus}','{$state}','{$localgov}','{$nationality}','{$hobbies}')";
