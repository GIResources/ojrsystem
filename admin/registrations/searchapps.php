...
<title>Registered Applicant Search</title>
<!-- STYLES & JQUERY 
================================================== -->
...
<p align="center" style="padding-left: 10px; padding-right:10px; color: blue; text:center; "><form action="app_search.php" method="POST" >
<p align="center"><input type="text" name="query" style="width:40%;"placeholder=" &nbsp;&nbsp;Search For Registered Applicants Here and Press Enter Key! " /> 
...
$app_lastname=($_POST['lastname']);


$app_firstname= mysql_real_escape_string(htmlspecialchars($_POST['firstname']));

$app_lastname = mysql_real_escape_string(htmlspecialchars($_POST['lastname']));
   ...
   @ $query = $_POST['query']; 
    ...
    if(strlen($query) >= $min_length){ // if query length is more or equal minimum length then
        ...
        $raw_results = mysqli_query($connection, "SELECT * FROM register_applicants
        ...
        if(mysqli_num_rows($raw_results) > 0){ // if one or more rows are returned do following
            ...
            while($results = mysqli_fetch_array($raw_results)){
				...
				echo "<tr> <th>ID</th> <th>Applicant JOB ID</th> <th>Title</th> <th>First Name</th><th>Last Name</th><th>Initials</th><th>Phone Contact</th> <th>Email Address</th><th>Nationality</th> <th>Gender</th> <th>Registration Date</th></tr>";
            // $results = mysql_fetch_array($raw_results) puts data from database into array, while it's valid it does the loop
			 ...
			 // . $row['id'] . '</td>';
				...
				echo '<td>'.$results['initials'].'</td>';
				echo '<td>'.$results['phone'].'</td>';
				echo '<td>'.$results['email'].'</td>';
				echo '<td>'.$results['nationality'].'</td>';
